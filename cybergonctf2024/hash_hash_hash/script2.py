...
val = rol(val,4)
key_arr = ['\x31\x3c\xbe\xab','\x55\xe1\x78\xac','\xb9\x7a\xc0\x6d','\x84\xe9\xb7\x7d','\x83\x5b\xd9\x8a','\x63\xad\x94\x43','\x5f\xb3\x5d\xfd','\x98\x30\x84\xd1']
key_1 = int("".join(f"{ord(c):02x}" for c in key_arr[-5]),16)
xor_edx_rsp_plus_rcx =   val ^ key_1
temp1 = rol(key,0xd)
temp2 = key & 0xf
a_hex = int("".join(f"{ord(c):02x}" for c in a[:4]),16)
b_hex = int("".join(f"{ord(c):02x}" for c in key_arr[-6]),16)
interim_2 = (a_hex + b_hex)&(2**32 - 1)
ror_eax_7 = ror(int("".join(f"{ord(c):02x}" for c in a[:4]),16),7)
ror_edi_cl = ror(interim_2,temp2)
key_3 = int("".join(f"{ord(c):02x}" for c in key_arr[-7]),16)
